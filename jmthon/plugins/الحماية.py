...
# ترجمه وكتابة فريق جمثون
@jmthon.on(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@jmthon.on(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@jmthon.on(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق جمثون
...
@jmthon.on(pattern="(س|سماح)(?:\s|$)([\s\S]*)")
...
@jmthon.on(pattern="الخاص بلوك(?:\s|$)([\s\S]*)")
async def block_p_m(event):
    if gvarstatus("pmpermit") is None:
...
@jmthon.on(pattern="انبلوك(?:\s|$)([\s\S]*)")
