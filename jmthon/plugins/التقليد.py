...
@jmthon.on(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@jmthon.on(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@jmthon.on(incoming=True, edited=False)
